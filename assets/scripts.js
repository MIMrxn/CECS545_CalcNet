...
class MachineI {
	...
	draw(curr_stmt) {
		...
		/*
		if(this.results.length != 5) {
			console.log("This is the results length in draw: "+this.results.length+"\n");
			window.requestAnimationFrame(this.draw());
		}
		*/
	}
...
class MachineD {
	...
	async load(letter) {
		if(this.vars_array[letter] === null) {
			...
			this.draw("Load", letter, loaded_value);
			return loaded_value;
		}
	}
	...
	draw(msg_type, letter, const_value) {
		var machines_canvas = document.getElementById("machines_canvas");
		var ctx = machines_canvas.getContext("2d");
	...
	async run() {
		...
		for(var i=0; i<factors.length; i++) {
			...
			if (isConstant(factor)) {
				...
				product *= factor;
				this.draw(factor, factor, product);
...
class MachineP {
	...
	async run() {
		...
		this.draw(this.expression, pow_result);
		return pow_result;
	}

	draw(pow_expr, result) {
		var machines_canvas = document.getElementById("machines_canvas");
		var ctx = machines_canvas.getContext("2d");
